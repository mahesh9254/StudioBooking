    ...
    public class BookingDTO
    {
        ...
        public double TotalHours
        {
            get
            {
                ...
                //var startDate = DateTime.ParseExact(BookingDate + " " + StartTime, Defaults.DefaultDateTime24Format, CultureInfo.InvariantCulture);
                //var endDate = DateTime.ParseExact(BookingEndDate + " " + EndTime, Defaults.DefaultDateTime24Format, CultureInfo.InvariantCulture);
                //return (endDate - startDate).TotalHours;

                var splitStartDate = BookingDate.Split('-').Select(Int32.Parse).ToList();
                DateTime dtStartFormatted = new(splitStartDate[2], splitStartDate[1], splitStartDate[0]);
                var splitEndDate = BookingEndDate.Split('-').Select(Int32.Parse).ToList();
                DateTime dtEndFormatted = new(splitEndDate[2], splitEndDate[1], splitEndDate[0]);

                var startDate = dtStartFormatted.Add(TimeSpan.Parse(StartTime));
                var endDate = dtEndFormatted.Add(TimeSpan.Parse(EndTime));
                return (endDate - startDate).TotalHours;
            }
        }
