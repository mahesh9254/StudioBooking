...
namespace StudioBooking.Areas.Admin.Controllers
{
    ...
    public class BookingController : BaseController
    {
        ...
        public async Task<IActionResult> ScheduleChangeRequest([FromBody] BookingDTO modal)
        {
            try
            {
                ...
                var request = new ScheduleRequest
                {
                    ...
                    RequestDate = DateTime.ParseExact(modal.BookingDate, Defaults.DefaultDateFormat, null),//DateTime.Parse(modal.BookingDate),
                    RequestEndDate = DateTime.ParseExact(modal.BookingEndDate, Defaults.DefaultDateFormat, null),//DateTime.Parse(modal.BookingDate),
        ...
        private async Task<Booking> SetBooking(BookingDTO booking)
        {
            ...
            var splitStartDate = booking.BookingDate.Split('-').Select(Int32.Parse).ToList();
            DateTime dtStartFormatted = new(splitStartDate[2], splitStartDate[1], splitStartDate[0]);
            var splitEndDate = booking.BookingEndDate.Split('-').Select(Int32.Parse).ToList();
            DateTime dtEndFormatted = new(splitEndDate[2], splitEndDate[1], splitEndDate[0]);

            var startDate = dtStartFormatted.Add(TimeSpan.Parse(booking.StartTime));
            var endDate = dtEndFormatted.Add(TimeSpan.Parse(booking.EndTime));
            var totalHours = (endDate - startDate).TotalHours;
        ...
        [HttpGet]
        public async Task<IActionResult> ExportAllBooking()
        {
            try
            {
                var bookings = await BookingViewModel.GetBookings(_context);

                #region Create Excel and Download
                using var workbook = new XLWorkbook();
                var worksheet = workbook.Worksheets.Add("Users");
                var currentRow = 1;
                worksheet.Cell(currentRow, 1).Value = "Booking ID";
                worksheet.Cell(currentRow, 2).Value = "Customer Name";
                worksheet.Cell(currentRow, 3).Value = "Studio";
                worksheet.Cell(currentRow, 4).Value = "Service Name";
                worksheet.Cell(currentRow, 5).Value = "Start Date Time";
                worksheet.Cell(currentRow, 6).Value = "End Date Time";
                worksheet.Cell(currentRow, 7).Value = "Hours";
                worksheet.Cell(currentRow, 8).Value = "Rate Per Hour";
                worksheet.Cell(currentRow, 9).Value = "Amount";
                worksheet.Cell(currentRow, 10).Value = "Status";
                worksheet.Cell(currentRow, 11).Value = "Payment";

                foreach (var user in bookings)
                {
                    currentRow++;
                    worksheet.Cell(currentRow, 1).Value = user.Booking.BookingId;
                    worksheet.Cell(currentRow, 2).Value = user.Customer.Name;
                    worksheet.Cell(currentRow, 3).Value = user.ServicePrice.CategoryName;
                    worksheet.Cell(currentRow, 4).Value = user.ServicePrice.ServiceName;
                    worksheet.Cell(currentRow, 5).Value = user.Booking.BookingDate + " " + user.Booking.StartTime;
                    worksheet.Cell(currentRow, 6).Value = user.Booking.BookingEndDate + " " + user.Booking.EndTime;
                    worksheet.Cell(currentRow, 7).Value = user.Booking.TotalHours;
                    worksheet.Cell(currentRow, 8).Value = user.Booking.RatePerHour;
                    worksheet.Cell(currentRow, 9).Value = user.Booking.Total;
                    worksheet.Cell(currentRow, 10).Value = Convert.ToString((BookingStatus)user.Booking.BookingStatus);
                    worksheet.Cell(currentRow, 11).Value = Convert.ToString((PaymentStatus)user.Booking.PaymentStatus);
                }
                ...
                return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Studio_Booking_" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".xlsx");
                #endregion
            }
            catch (Exception ex)
            {
                _logger.LogError("ExportAllBooking", ex);
                return Ok(new { result = false, err = ex.Message });
            }
        }
    }
